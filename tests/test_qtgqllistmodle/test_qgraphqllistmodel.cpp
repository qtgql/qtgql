...
// TODO: test this with some generated code.
// typedef std::unique_ptr<FooQObject> UniqueFoo;
// using namespace qtgql;
//
// typedef bases::ListModelABC<std::shared_ptr<FooQObject>>
// SampleGraphQLListModel;
//
// struct CompleteSpy {
//  QSignalSpy* about_to;
//  QSignalSpy* after;
...
//  std::unique_ptr<QList<UniqueFoo>> p__init_vec =
//      std::make_unique<std::vector<UniqueFoo>>();
//  std::vector<UniqueFoo> init_list_copy;
//  for (const auto& a : {"foo", "bar", "baz"}) {
//    auto obj = std::make_unique<FooQObject>(a);
//    p__init_vec->push_front(std::move(obj));
//    init_list_copy.push_front(obj);
//  }
//  auto model_with_data =
//      SampleGraphQLListModel{nullptr, std::move(p__init_vec)};
