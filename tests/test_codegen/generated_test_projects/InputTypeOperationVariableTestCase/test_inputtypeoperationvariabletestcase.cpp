...
TEST_CASE("InputTypeOperationVariableTestCase", "[generated-testcase]") {
  ...
  create_post->set_variables(
      {new InputTypeOperationVariableTestCase::CreatePostInput{old_post_content,
                                                               "Post header"}});
  create_post->fetch();
  ...
  SECTION("test update") {
    ...
    update_post->set_variables(
        {new InputTypeOperationVariableTestCase::ModifyPostContentInput(
            post->get_id(), new_content)});
