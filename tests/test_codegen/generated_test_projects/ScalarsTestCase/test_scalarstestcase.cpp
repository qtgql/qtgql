...
TEST_CASE("Test GraphQLOverHttp as env") {
  ...
  auto suffix = SCHEMA_ADDR.split("/").last();
  auto gql_over_http = std::unique_ptr<gqloverhttp::GraphQLOverHttp>{
      new gqloverhttp::GraphQLOverHttp(test_utils::get_http_server_addr(suffix),
                                       {})};
  auto env_ =
      std::make_shared<bases::Environment>(ENV_NAME, std::move(gql_over_http));
