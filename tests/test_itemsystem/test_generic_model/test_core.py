...
from qtgql.itemsystem import GenericModel, role, RoleDoesNotExist, BaseRoleDefined
from tests.test_itemsystem.fixtures import FullClass, init_dict_fullClass, WithChild, init_dict_withChild, \
    NORMAL_GQL_CAMELIZED, NO_CAMEL_CASE, CHILD
...
def test_child_has_parent_model(schema, full_model):
    parent_model: GenericModel[WithChild] = WithChild.Model(schema=schema, data=[init_dict_withChild() for _ in range(3)])
