...
class _QGraphQLListModelMixin : public QAbstractListModel {
 ...
 protected:
  static const int c_object_role = Qt::UserRole + 1;
...
class QGraphQLListModelABC : public _QGraphQLListModelMixin {
 ...
 public:
  ...
  void append(const T_sharedQObject& shared_obj_ref) {
    beginInsertRows(invalid_index(), m_count, m_count);
  ...
  void clear() {
    if (!m_data.isEmpty()) {
      beginRemoveRows(invalid_index(), 0, m_count - 1);
      m_data->clear();
      update_count();
      endRemoveRows();
    }
  ...
  bool removeRows(int row, int count,
    ...
    if ((row + count) <= m_count) {
      beginRemoveRows(invalid_index(), row, count);
