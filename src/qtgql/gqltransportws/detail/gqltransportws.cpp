...
GqlTransportWs::GqlTransportWs(GqlTransportWsClientSettings settings)
    : m_settings(std::move(settings)), QObject::QObject(nullptr) {
  m_ws = new QWebSocket();
  m_ws->setParent(this);
...
void GqlTransportWs::on_gql_next(const GqlTrnsWsMsgWithID &message) {
  if (message.has_payload()) {
    m_connected_handlers[message.op_id]->on_next(
...
void GqlTransportWs::on_gql_error(const GqlTrnsWsMsgWithID &message) {
  if (message.has_errors()) {
    const auto handler = m_connected_handlers.at(message.op_id);
...
void GqlTransportWs::on_gql_complete(const GqlTrnsWsMsgWithID &message) {
  m_connected_handlers[message.op_id]->on_completed();
...
void GqlTransportWs::on_gql_ack() {
  ...
  while (i != m_pending_handlers.end()) {
    if (const auto &[execution_id, handler] = (*i);
        execution_id == handler->get_execution_id()) {
      qDebug() << "re-executing handler with id: " << execution_id;
      execute_impl(execution_id, handler);
      if (m_connected_handlers.contains(execution_id)) {
...
void GqlTransportWs::onTextMessageReceived(const QString &message) {
  if (const auto raw_data = QJsonDocument::fromJson(message.toUtf8());
      raw_data.isObject()) {
    if (const auto data = raw_data.object(); data.contains("id")) {
      // Any that contains ID is directed to a single handler.
      const auto op_message = GqlTrnsWsMsgWithID(data);
      // do stuff only if the handlers current execution id equals the id we
      // have
      if (m_connected_handlers.contains(op_message.op_id) &&
          m_connected_handlers[op_message.op_id]->get_execution_id() ==
              op_message.op_id) {
        if (const auto message_type = op_message.type;
            message_type == PROTOCOL::NEXT()) {
      ...
      // otherwise finnish this session by calling COMPLETE
      else {
        send_message(
            GqlTrnsWsMsgWithID(PROTOCOL::COMPLETE(), op_message.op_id));
        m_connected_handlers.erase(op_message.op_id);
      }

    } else {
      const auto conn_message = BaseGqlTrnsWsMsg(data);
      const auto message_type = conn_message.type;
...
void GqlTransportWs::execute(
  ...
  execute_impl(handler->get_execution_id(), handler);
}
void GqlTransportWs::execute_impl(
    const QUuid &execution_id,
    const std::shared_ptr<bases::HandlerABC> &handler) {
  // if GQL_ACK and connected
  // send a message and delete the pending handler, it is now "connected".
  if (!m_connected_handlers.contains(execution_id)) {
    if (gql_is_valid()) {
      m_connected_handlers[execution_id] = handler;
      send_message(GqlTrnsWsMsgWithID(handler->message(), execution_id));
    } else if (!m_pending_handlers.contains(execution_id)) {
      m_pending_handlers[execution_id] = handler;
    }
  }
}
