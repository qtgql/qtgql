...
import httpx
from releasefile import parse_release_file, ReleasePreview
import githubref
...
def update_change_log(current_changes: str, version: str) -> None:
    ...
    PATHS.CHANGELOG.write_text(textwrap.dedent(f"{main_header}"
                                               f"{this_header}"
                                               f"{current_changes}\n\n"
                                               f"{previous}"
                                               ), encoding='utf-8')
...
if __name__ == "__main__":
    ...
    current_contributor = get_last_commit_contributor(bot_token)
