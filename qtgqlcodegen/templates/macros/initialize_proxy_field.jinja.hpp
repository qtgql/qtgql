...
{% elif field.type.is_model and not field.type.of_type.is_builtin_scalar %}
    ...
    auto init_list_👉 field.name 👈 =  std::vector<👉field.type.of_type.name👈*>();
    for (const auto & node: 👉 instance_of_concrete 👈){
    init_list_👉 field.name 👈.push_back(new 👉field.type.of_type.name👈(👉operation_pointer👈, node));
    }
    👉field.private_name👈 = new qtgql::bases::ListModelABC<👉 field.type.of_type.property_type 👈>(this, std::move(init_list_👉 field.name 👈));
    {% elif field.type.is_model.of_type.is_queried_union or field.type.is_model.of_type.is_queried_interface%}
    auto init_list_👉 field.name 👈 =  std::vector<👉field.type.of_type.type_name()👈>();
    for (const auto & node: 👉 instance_of_concrete 👈){
        ...
        {% set do_on_meets -%}
        init_list_👉 field.name 👈.push_back(qobject_cast<👉 field.type.of_type.property_type 👈>(new 👉choice.type_name()👈(👉operation_pointer👈, std::static_pointer_cast<👉 choice.concrete.name 👈>(node))));
        {% endset -%}
